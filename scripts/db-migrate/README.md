...
### Гонка с живым сайтом (users.email, orders.orderNumber)

Пока миграция гоняется, сайт продолжает принимать регистрации и заказы — это
и есть весь смысл «переходного периода». `adoptExistingByEmail`
(users.migration.ts) и `resolveFreeOrderNumber` (orders.migration.ts)
устроены как «проверить, потом записать»: между проверкой и записью в
Postgres может успеть прилететь настоящая запись с сайта, попадающая ровно в
то же уникальное значение (email или orderNumber). Тогда `create()` падает на
unique-constraint — не потому что в данных ошибка, а потому что проверка на
миллисекунды устарела.

Обе миграции ловят именно эту ошибку (по имени поля из
`extractUniqueConstraintFieldPaths`, см. `core/errors.ts`) и повторяют
попытку один раз в рамках того же прогона: `users` заново ищет владельца по
email (на этот раз находит и связывает вместо create), `orders` заново
проверяет свободный номер (на этот раз видит занятость и подбирает суффикс
`-L`). Без ретрая такая запись просто падала бы в `failed` до следующего
запуска — это не потеря данных (см. «Идемпотентность» выше), но означало бы
лишний ручной перезапуск ради каждой залетевшей гонки.

Если после ретрая запись всё равно в `ERROR` — это уже не гонка (окно между
проверкой и записью — миллисекунды, вторая гонка в него подряд практически
невозможна), а настоящая проблема с данными: например мусорный email в
старой БД, не проходящий формат (нелатинские символы, пробелы) — такое
ретраем не лечится, запись нужно поправить в старой БД или перенести вручную.
