...
/** Строгий набор без раскрытия зависимостей — используется с --no-deps. */
function exactSlugSet(
  modules: MigrationModule[],
  onlySlugs: string[],
): Set<string> {
  const bySlug = new Map(modules.map((m) => [m.slug, m]));
...
export async function runMigrations(
  ...
  const runSet = options.only
    ? options.noDeps
      ? exactSlugSet(modules, options.only)
  ...
  for (const mod of ordered) {
    ...
    try {
      ...
      // Инвалидируем закэшированные карты id — после прогона в БД
      // появились новые записи, а последующие миграции этого же запуска
      // (в т.ч. подтянутые через --only по зависимостям) должны их видеть.
      //
      // Раньше здесь стояло idMapCache.delete(mod.slug), но ключ кэша —
      // имя КОЛЛЕКЦИИ (getIdMap("pickup-points")), а mod.slug — имя
      // МИГРАЦИИ ("pickupPoints"). Для всех миграций, где они не совпадают
      // (pickupPoints, transportCompanies, userConsents), сброс был
      // no-op'ом: карта, построенная до прогона, оставалась в кэше без
      // новых записей, и ссылки на них молча не резолвились. Сейчас это не
      // стреляет только потому, что порядок вызовов случайно удачный —
      // такую мину проще убрать, чем поддерживать соответствие имён.
      // Стоимость — перестроение карты при следующем обращении, копейки на
      // фоне самой миграции.
      idMapCache.clear();
      log.info(
