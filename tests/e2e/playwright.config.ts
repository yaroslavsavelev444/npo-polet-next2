	...
	webServer: [
		...
		},
		{
			...
			env: {
				...
				// .env у запускающего.
				//
				// Тесты оформляют заказы на заведомо несуществующие адреса
				// (e2e.checkout@example.test), а каждое такое письмо
				// возвращается отлупом в настоящий почтовый ящик владельца
				// SMTP-аккаунта: десятки уведомлений за один прогон, и репутация
				// отправителя портится по-настоящему.
				//
				// Информативности это не стоит ничего: EmailService при
				// EMAIL_ENABLED=false отдаёт вызывающему успех и пишет в лог
				// шаблон и получателей, то есть факт «уведомление сработало»
				// по-прежнему виден и проверяем.
				EMAIL_ENABLED: "false",
			},
			stdout: "ignore",
			stderr: "pipe",
