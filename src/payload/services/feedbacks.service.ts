...
export interface GetFeedbacksOptions {
  status?: 'new' | 'in_progress' | 'resolved' | 'closed' | 'duplicate' | 'wont_fix'
...
function getFeedbacksCacheKey(options?: GetFeedbacksOptions): string {
  const { status, type, priority, user, assignedTo, tags, search, sort, limit, page, depth } = options || {}
...
async function fetchFeedbacksByUser(userId: string, options: Omit<GetFeedbacksOptions, 'user'> = {}) {
  return fetchFeedbacks({ ...options, user: userId })
}

export const getCachedFeedbacksByUser = (userId: string, options?: Omit<GetFeedbacksOptions, 'user'>) => {
