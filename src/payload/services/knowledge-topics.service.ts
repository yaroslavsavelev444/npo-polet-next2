...
function getKnowledgeTopicsCacheKey(options?: GetKnowledgeTopicsOptions): string {
  const { published, featured, tags, search, sort, limit, page, depth } = options || {}
...
export const getCachedKnowledgeTopics = (options?: GetKnowledgeTopicsOptions) => {
  ...
  return unstable_cache(
    fetchFn,
    [getKnowledgeTopicsCacheKey(options)],
    { tags: ['knowledge-topics'], revalidate: false }
  )()
}

async function fetchKnowledgeTopicById(id: string): Promise<KnowledgeTopic | null> {
  const payload = await getPayloadInstance()
  const result = await payload.find({
    collection: 'knowledge-topics',
    where: { id: { equals: id } },
...
async function fetchKnowledgeTopicBySlug(slug: string): Promise<KnowledgeTopic | null> {
  const payload = await getPayloadInstance()
  const result = await payload.find({
    collection: 'knowledge-topics',
