...
function getCompaniesCacheKey(options?: GetCompaniesOptions): string {
  const { user, taxNumber, companyName, sort, limit, page, depth } = options || {}
...
export const getCachedCompanies = (options?: GetCompaniesOptions) => {
  ...
  return unstable_cache(
    fetchFn,
    [getCompaniesCacheKey(options)],
    { tags: ['companies'], revalidate: false }
  )()
}
...
async function fetchCompanyByTaxNumber(taxNumber: string): Promise<Company | null> {
  const payload = await getPayloadInstance()
  const result = await payload.find({
    collection: 'companies',
...
export const getCachedCompanyByTaxNumber = (taxNumber: string) => {
  ...
  return unstable_cache(
    fetchFn,
    [`company-tax-${taxNumber}`],
    { tags: ['companies'], revalidate: false }
  )()
}

async function fetchCompaniesByUser(userId: string, options: Omit<GetCompaniesOptions, 'user'> = {}) {
  return fetchCompanies({ ...options, user: userId })
}

export const getCachedCompaniesByUser = (userId: string, options?: Omit<GetCompaniesOptions, 'user'>) => {
