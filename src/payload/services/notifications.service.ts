...
function getNotificationsCacheKey(options?: GetNotificationsOptions): string {
  const { user, type, isRead, pushStatus, sort, limit, page, depth } = options || {}
...
export const getCachedNotifications = (options?: GetNotificationsOptions) => {
  ...
  return unstable_cache(
    fetchFn,
    [getNotificationsCacheKey(options)],
    { tags: ['notifications'], revalidate: false }
  )()
}
...
async function fetchNotificationsByUser(userId: string, options: Omit<GetNotificationsOptions, 'user'> = {}) {
  return fetchNotifications({ ...options, user: userId })
}

export const getCachedNotificationsByUser = (userId: string, options?: Omit<GetNotificationsOptions, 'user'>) => {
