...
function buildTransportCompaniesWhere(
  options: GetTransportCompaniesOptions,
): Where {
  const where: Where = {};
...
function getTransportCompaniesCacheKey(
  options?: GetTransportCompaniesOptions,
): string {
...
export const getCachedTransportCompanies = (
  options?: GetTransportCompaniesOptions,
) => {
  ...
  return unstable_cache(fetchFn, [getTransportCompaniesCacheKey(options)], {
    tags: ["transport-companies"],
    revalidate: false,
  })();
};

async function fetchTransportCompanyById(
  id: string,
...
export const getCachedTransportCompanyById = (id: string) => {
  ...
  return unstable_cache(fetchFn, [`transport-company-${id}`], {
    tags: ["transport-companies"],
    revalidate: false,
  })();
};
