...
function getConsentsCacheKey(options?: GetConsentsOptions): string {
  const { isActive, isRequired, slug, sort, limit, page, depth } =
    options || {};
...
export const getCachedConsents = (options?: GetConsentsOptions) => {
  ...
  return unstable_cache(fetchFn, [getConsentsCacheKey(options)], {
    tags: ["consents"],
    revalidate: false,
  })();
};
...
export const getCachedConsentBySlug = (slug: string) => {
  ...
  return unstable_cache(fetchFn, [`consent-${slug}`], {
    tags: ["consents"],
    revalidate: false,
  })();
};
...
export const getCachedConsentById = (id: string) => {
  ...
  return unstable_cache(fetchFn, [`consent-${id}`], {
    tags: ["consents"],
    revalidate: false,
  })();
};
