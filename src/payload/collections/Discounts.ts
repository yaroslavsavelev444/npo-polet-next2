...
export const Discounts: CollectionConfig = {
  ...
  admin: {
    useAsTitle: 'name',
    group: 'E-Commerce',
    defaultColumns: ['name', 'type', 'discountPercent', 'isActive', 'priority', 'updatedAt'],
  },
  access: {
    read: () => true,
  ...
  fields: [
    ...
    { name: 'totalUses', type: 'number', defaultValue: 0, admin: { readOnly: true } },
    { name: 'totalDiscountAmount', type: 'number', defaultValue: 0, admin: { readOnly: true } },
      ...
      hooks: {
        afterRead: [
          ({ data }) => {
            ...
            if (!data.isUnlimited && data.endAt && now > new Date(data.endAt)) return false
            return true
          },
        ],
      },
