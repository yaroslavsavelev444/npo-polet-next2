...
const versioningHook = async ({ data, originalDoc, operation }: any) => {
  ...
  const urlChanged = data.documentUrl && data.documentUrl !== originalDoc.documentUrl
  ...
  const prevEntry = {
    ...
    changeDescription: data._changeDescription ?? 'Автоматическое сохранение истории',
  }

  data.history = [...(originalDoc.history ?? []), prevEntry]

  // 2. Инкрементируем patch
  const [major, minor, patch] = (originalDoc.version ?? '1.0.0').split('.').map(Number)
