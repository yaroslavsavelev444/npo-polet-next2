...
export type CheckoutActionErrorCode =
	...
	| "CART_INVALID"
	/**
	 * В корзине лежит товар, который больше нельзя заказать.
	 *
	 * Отдельный код, а не CART_INVALID: заказ не создаётся не потому, что
	 * что-то введено неверно, а потому, что состав корзины изменился без
	 * участия покупателя. Исправляется он одним действием — убрать позицию, —
	 * и сообщение обязано вести именно туда.
	 */
	| "CART_HAS_UNAVAILABLE"
	| "VALIDATION_ERROR"
	/**
