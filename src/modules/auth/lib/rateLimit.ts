...
export const RATE_LIMITS = {
	...
	// Форма на странице контактов. Лимит строже, чем у feedback: сюда пишут
	// один раз с конкретным вопросом, а не отправляют серию отчётов, — три
	// сообщения за полчаса с одного адреса покрывают и повтор после опечатки,
	// и «дописал, что забыл». fail-open по тем же причинам, что и остальные
	// публичные формы: сбой Redis не должен закрывать единственный канал
	// связи на сайте.
	contactRequest: (ip: string) =>
		checkRateLimit(`contact_request:${ip}`, 3, 30 * 60 * 1000),
