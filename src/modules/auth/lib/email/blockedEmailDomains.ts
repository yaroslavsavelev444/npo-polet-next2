/**
 * Блок-лист доменов, которые НИКОГДА не должны проходить регистрацию.
 *
 * Это техническая, а не территориальная проверка: она отсекает адреса, на
 * которые физически невозможно доставить письмо с кодом подтверждения, и
 * потому остаётся в силе после того, как ограничение регистрации российскими
 * почтовыми доменами было снято (см. emailValidator.ts).
 *
 * Разделён по смыслу:
